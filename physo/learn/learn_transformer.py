...
def learner ( model,
    ...
    for epoch in range (n_epochs):
        ...
        for i in range (max_time_step):
            ...
            output = model(observations,    # (batch_size, output_size)
                                            )
           ...
           # 0 protection so there is always something to sample
            ...
            # lp = logit[torch.arange(action.shape[0]), action]           # shape (B,)
            # print("log_prob example:", lp[0].item())
            # print(logit[0])

            # ------------ ACTION ------------

            # Saving action n°i
            logits       .append(logit) #HERE
            actions      .append(action) #HERE
